...
func Test_convert_read_int64_with_quote(t *testing.T) {
	should := require.New(t)
	iter := jsoniter.ParseString(jsoniter.ConfigConvertStringNumber, "\"123456789101112\"")
	v := iter.ReadUint64()
	should.Nil(iter.Error)
	should.Equal(v, uint64(123456789101112))

	iter = jsoniter.ParseString(jsoniter.ConfigConvertStringNumber, "\"123456789101112\"")
	v2 := iter.ReadInt64()
	should.Nil(iter.Error)
	should.Equal(v2, int64(123456789101112))
...
func Test_read_string_with_quote(t *testing.T) {
	should := require.New(t)
	input := `[123.0,4560, "789.453", 12, "34567"]`
	val := make([]string, 0)
	err := jsoniter.ConfigConvertStringNumber.UnmarshalFromString(input, &val)
	should.Nil(err)
	should.Equal(5, len(val))
	should.Equal([]string{"123.0", "4560", "789.453", "12", "34567"}, val)
...
func Test_convert_string_number(t *testing.T) {
	should := require.New(t)
	s := `{"ii": 1234, "si": "1234", "ll": 908374832, "sl": "908374832", "ll2": 908374832,
			"ll3": "908374832", "ff": 3.1415926, "sf": "3.1415926", "bf": false, "bt": true,
			"inner": {  "iii": 4321  , "isi": "4321", "ill": -108374832, "isl": "-108374832", "ill2": -108374832,
				"ill3": "-108374832" , "iff": -113.1415926, "isf": "-113.1415926"},
